...
export default function PortfolioCard({ data, index, onDelete }: CardProps) {
  ...
  return (
    <div className="bg-[#0b1120] border border-white/[0.05] rounded-[32px] p-6 shadow-xl relative group">
      ...
      {/* PRINT BO'LADIGAN ASOSIY BLOK (CSS print effektlari berilgan) */}
      <div 
        ref={contentRef} 
        className="p-8 rounded-2xl bg-[#05070f] border border-white/[0.04] flex flex-col md:flex-row gap-8 items-start relative print:bg-white print:text-black print:p-0 print:border-none"
      >
        {/* AVATAR/RASM */}
        ...
        <div className="flex-grow w-full text-center md:text-left">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 border-t border-white/[0.05] pt-4 print:border-black/10">
            <div>
              <span className="text-[10px] text-white/30 uppercase font-bold tracking-wider block mb-1 print:text-black/40">Aloqa</span>
              <span className="text-sm text-white/80 print:text-black font-medium">{data.phone || "Kiritilmagan"}</span>
            </div>
            {data.skills && (
              <div>
                <span className="text-[10px] text-white/30 uppercase font-bold tracking-wider block mb-1 print:text-black/40">Ko'nikmalar</span>
          ...
          {/* STARTUP LOYIHA CHIZMASI */}
          {data.startupName && (
            <div className="mt-5 p-4 rounded-xl bg-indigo-600/5 border border-indigo-500/10 print:bg-black/[0.02] print:border-black/10 text-left">
              <h4 className="text-xs font-bold text-indigo-400 uppercase tracking-widest mb-1 print:text-indigo-700">
                🚀 Start-up Loyihasi: {data.startupName}
              </h4>
